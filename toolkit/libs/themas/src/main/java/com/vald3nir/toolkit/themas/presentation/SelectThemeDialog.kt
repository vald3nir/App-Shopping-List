...
import com.vald3nir.toolkit.themas.R
...
private fun SelectThemeDialog(
    ...
    onDismiss: () -> Unit = {},
    onChangeThemeBrand: (themeBrand: ThemeBrandEnum2?) -> Unit = {},
    ...
    AnimatedVisibility(visible = !settings.useDynamicColor) {
        Column {
            ...
            ToolkitRadioButtonGroup(
                items = ThemeBrandEnum2.entries.map { it.key },
                viewType = ToolkitRadioButtonGroupType.GRID,
                selectedValue = settings.themeBrand?.key,
                onItemSelected = {
                    onChangeThemeBrand(ThemeBrandEnum2.fromKey(it))
                }
            )
        }
    }
...
private fun PreviewDialogDynamicColor() {
    ToolkitTheme {
        SelectThemeDialog(
            selectThemeUiState = SelectThemeUiState.Success(
                AppThemeDTO(
                    themeBrand = ThemeBrandEnum2.BLUE,
