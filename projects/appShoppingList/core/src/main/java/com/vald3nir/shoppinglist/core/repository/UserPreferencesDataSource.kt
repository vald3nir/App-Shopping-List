...
private val Context.userPreferencesStore by preferencesDataStore(name = "user_prefs")

private object DataStoreKeys {
    // todo valdenir remover classe
    val THEME_BRAND = stringPreferencesKey("theme_brand")
    val DARK_THEME = stringPreferencesKey("dark_theme")
    val USE_DYNAMIC_COLOR = booleanPreferencesKey("use_dynamic_color")
}

internal class UserPreferencesDataSource(private val context: Context) {

    val userData: Flow<AppThemeDTO> = context.userPreferencesStore.data
        .catch { exception ->
        ...
        .map { prefs ->
            AppThemeDTO(
                themeBrand = ThemeBrandEnum.valueOf(prefs[DataStoreKeys.THEME_BRAND] ?: ThemeBrandEnum.DEFAULT.name),
                themeConfigEnum = UIThemeConfigEnum.valueOf(prefs[DataStoreKeys.DARK_THEME] ?: UIThemeConfigEnum.FOLLOW_SYSTEM.name),
    ...
    suspend fun setThemeBrand(themeBrand: ThemeBrandEnum) {
        context.userPreferencesStore.edit { prefs ->
            prefs[DataStoreKeys.THEME_BRAND] = themeBrand.name
        }
    }
    ...
    suspend fun setDarkThemeConfig(config: UIThemeConfigEnum) {
        context.userPreferencesStore.edit { prefs ->
            prefs[DataStoreKeys.DARK_THEME] = config.name
        }
    }
}